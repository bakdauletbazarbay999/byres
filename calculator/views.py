...
def download_check(request, order_id):
    ...
    lines = []
    lines.append("===== BY RESTAURANT =====")
    lines.append(f"Тапсырыс №: {order.id}")
    lines.append(f"Уақыты: {order.created_at.strftime('%d.%m.%Y %H:%M')}")
    lines.append("------------------------")
    lines.append("Тағамдар:")
    ...
    lines.append("------------------------")
    lines.append(f"ЖАЛПЫ: {total} ₸")
    lines.append("------------------------")
    lines.append("Рақмет!")

    response = HttpResponse("\n".join(lines), content_type='text/plain')
    response['Content-Disposition'] = f'attachment; filename=check_{order.id}.txt'
